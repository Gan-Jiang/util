...
Insert, delete and search: Average case: O(log n)
'''
...
class TreeNode:
    ...
    def findPreSuccessor(self):
        presucc = None
        if self.hasLeftChild():
            presucc = self.rightChild.findMax()
        else:
            if self.parent:
                if self.isRightChild():
                    ...
                    self.parent.leftChild = None
                    presucc = self.parent.findPreSuccessor()
                    self.parent.leftChild = self
            '''
            p = self.parent
            T = self
            while p is not None and T.isLeftChild():
                T = p
                p = T.parent
            if p is None:
                return -1
